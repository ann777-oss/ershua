...
| P6 | 知乎登录与我的案源 | 0.5 天 | 收藏故事进入「我的案源」 |
...
## 9. P6 知乎登录与我的案源（0.5 天）

### 功能定义

**做**：玩家通过知乎 OAuth 登录；服务端读取该玩家近期收藏与收藏夹内容；与黑客松官方故事库交叉匹配；故事库页新增「我的案源 · 来自你的知乎收藏」分区。命中官方故事的收藏可一键进入现有故事库分流逻辑；未命中的收藏提供「尝试侦探化」兜底，直接用收藏条目的内容 ID 调用现有故事内容详情与流水线，成功即玩，失败如实提示。

**不做**：不改动主线《她从不下厨》与官方故事库的原有行为；不保存用户长期数据；不把知乎账号 token、App Key、Access Secret 或收藏详情写入前端持久化；不在无凭证环境展示不可用入口。

### 新增 API 契约

| 端点 | 请求 | 响应 | 说明 |
|---|---|---|---|
| `GET /api/auth/status` | Cookie: `zsid?` | `{configured:boolean, loggedIn:boolean, expiresIn:number}` | 静默检查登录能力与会话状态；未配置 OAuth 时返回 `configured:false` |
| `GET /api/auth/url` | — | `{url, redirectUri}`；未配置时 `503 {configured:false,error}` | 返回知乎授权页地址；state 为 10 分钟一次性防 CSRF 值 |
| `GET /api/auth/callback` | OAuth 回调参数：`authorization_code`（兼容 `code`）与可能的 `state` | HTML 结果页；成功时设置 HttpOnly Cookie `zsid` | 服务端完成 code 换 token；会话 TTL 24 小时 |
| `POST /api/auth/logout` | Cookie: `zsid?` | `{ok:true}` | 删除服务端会话并清除 cookie |
| `GET /api/me/cases` | Cookie: `zsid` | `{matched, others, favoriteTotal, fetchedAt}`；未登录 `401`；缺 Access Secret `503` | 读取用户收藏，按 `work_id == UrlToken` 或标题相等匹配官方故事；`others` 上限 10 |

### 安全边界

- `ZHIHU_OAUTH_APP_ID`、`ZHIHU_OAUTH_APP_KEY`、`ZHIHU_ACCESS_SECRET`、OAuth token 只存在服务端环境变量或内存会话中，禁止进入源码、前端响应、日志、截图和 commit。
- 前端只拿登录状态与可展示的案源摘要；不接收 App Key、Access Secret、access token。
- 会话使用 HttpOnly Cookie `zsid`，`SameSite=Lax`，24 小时过期；服务端内存 Map 无长期用户库。
- 未配置 OAuth 凭证时，`/api/auth/status` 明确返回 `configured:false`，前端完全隐藏登录入口，不影响主线与官方故事库。
- `/api/me/cases` 只代表当前登录用户的收藏；不得静默降级为 Access Secret 所属账号或任何模拟收藏。
- 官方补充 OAuth 文档提示回调可能不返回 `state`；真实登录联调前必须确认线上回调参数，并在安全边界内处理该差异。

### 验收标准

- 无新增凭证环境：`/api/auth/status` 返回 `configured:false`；故事库无「知乎登录」入口；`/api/me/cases` 返回 `401` 或缺配置错误；主线《她从不下厨》与官方故事库可正常进入。
- 配齐凭证环境：点击「知乎登录」跳转官方授权页；回调后写入会话；故事库 topbar 显示「已登录 · 退出」。
- 登录后故事库在主线卡片上方展示「我的案源」；加载中、错误、空收藏、重试状态均有可见反馈。
- `matched` 案源复用现有分流：已落盘 slot 直接进入游戏；未落盘走流水线。
- `others` 案源点击「尝试侦探化」后复用现有流水线；内容详情或流水线失败时按现有错误机制如实提示。
- 退出登录后清空会话与「我的案源」状态，回到未登录 UI。

### 阶段门
自检 → 无凭证冒烟 → 真实凭证联调 → 代码审查（secrets/错误提示/降级路径）→ 用户线上验收 → `git tag phase-p6`
...
## 13. v1.2 变更：定名「二刷」与战略决策固化（2026-09-08，用户拍板）
...
### 13.2 适配广度回应（对应"20 篇里悬疑只有 4 篇"的担忧）
...
- **P6 启动项**：OAuth 登录升级为「知乎登录 · 我的案源」——用户授权后读取其收藏，与官方故事库匹配并尝试侦探化收藏故事；凭证未配置时入口隐藏，保持主线与官方故事库稳定。

---

*版本 v1.3 ｜ 2026-09-14 ｜ v1.0 依据 PRD v1.0/design.md v1.1/数据资产 v1.0；v1.1 官方故事 API 混合接入；v1.2 定名「二刷」+ 战略决策固化（§12-13）；v1.3 新增 P6 知乎登录与我的案源（§9）*
